...
class ChoosePassion extends StatefulHookConsumerWidget {
  final Function? onSubmit;

  const ChoosePassion({super.key, this.onSubmit});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() => _ChoosePassionState();
}

class _ChoosePassionState extends ConsumerState<ChoosePassion> {
  ...
  void initState() {
    ...
    // Forcer la reconstruction des sélections après le premier build
    // pour s'assurer que les defaultActive sont bien appliqués
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        setState(() {
          // Force rebuild pour appliquer les sélections sauvegardées
        });
      }
    });
  }

  /// Vérifie qu'au moins 3 tags sont sélectionnés par catégorie
  bool _isValidSelection() {
    final passionCount = passionKey.currentState?.getActiveItems().length ?? 0;
    final fiestaCount = fiestaKey.currentState?.getActiveItems().length ?? 0;
    final musicCount = musicKey.currentState?.getActiveItems().length ?? 0;
    final drinkCount = drinkKey.currentState?.getActiveItems().length ?? 0;

    return passionCount >= 3 &&
        fiestaCount >= 3 &&
        musicCount >= 3 &&
        drinkCount >= 3;
  }
  ...
  String? _getValidationMessage() {
    try {
      final passionCount =
          passionKey.currentState?.getActiveItems().length ?? 0;
      final fiestaCount = fiestaKey.currentState?.getActiveItems().length ?? 0;
      final musicCount = musicKey.currentState?.getActiveItems().length ?? 0;
      final drinkCount = drinkKey.currentState?.getActiveItems().length ?? 0;

      if (passionCount < 3) return "app.validation.passion-minimum".tr();
      if (fiestaCount < 3) return "app.validation.fiesta-minimum".tr();
      if (musicCount < 3) return "app.validation.music-minimum".tr();
      if (drinkCount < 3) return "app.validation.drink-minimum".tr();
      return null;
    } catch (e) {
      // En cas d'erreur, ne pas afficher de message d'erreur pour éviter les boucles
      return null;
    }
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        sh(7),
        Text("${"utils.step".tr()} 2", style: AppTextStyle.black(26)),
        sh(22),
        CustomSearchBar(
          options: const [],
        ...
        MultiItemSelector(
          ...
          defaultActive: ref
              ...
              .data["favorite"]?["passion"],
          onTapOnItem: (isSelected, tag, activeItems) {
            setState(() {
        ...
        CTA.primary(
          ...
          onTap: _isValidSelection()
              ? () {
                  FocusManager.instance.primaryFocus?.unfocus();
                  final map = {};
                  map["passion"] = passionKey.currentState?.getActiveItems();

...
@RoutePage()
class UserProfilPage extends StatefulHookConsumerWidget {
  final String id;

  const UserProfilPage({super.key, @PathParam("id") required this.id});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() => _UserProfilPageState();
}

class _UserProfilPageState extends ConsumerState<UserProfilPage> {
  final ValueNotifier<int> page = ValueNotifier<int>(0);

  late Future<List<UserFiestaModel>?> userFiestaCreated;
  late Future<List<UserFiestaModel>?> userParticipatedFiesta;
  late Future<AppUserModel?> userData;
  ...
  void initState() {
    userData = FirebaseFirestore.instance
        ...
        .then((value) {
          ...
          return AppUserModel.fromJson(data).copyWith(
            id: widget.id,
            pictures:
                (data["profilImages"] as List<dynamic>?)
                    ?.map((e) => e as String)
                    .toList() ??
                const <String>[],
            // Mapper le champ 'favorite' vers 'tags' pour l'affichage des favoris
            tags: data["favorite"] != null
                ? PassionListing.fromJson(data["favorite"])
                : data["tags"] != null
                ? PassionListing.fromJson(data["tags"])
                : null,
          );
        })
        .catchError((_) => null);
          ...
          builder: (_, AsyncSnapshot<AppUserModel?> snapshot) {
            if (snapshot.hasError) {
              return Center(
                child: Padding(
                  ...
                  child: Text(
                    "Accès au profil refusé ou indisponible.",
            ...
            if (snapshot.hasData && snapshot.data != null) {
              ...
              return Column(
                children: [
                  sh(12),
                  _buildPrincipalData(context, userModel),
                  sh(2),
                  Center(
                    child: Wrap(
                      ...
                      children: [
                        ...
                        if (userModel.numberRecommandations != null) ...[
                          Row(
                            children: [
                              Text(" -  ", style: AppTextStyle.darkGray(14)),
                              SvgPicture.asset(
                              ...
                              sw(1),
                              Text(
                                "${userModel.numberRecommandations!.toInt().toString()} ${"utils.recommandation".plural(userModel.numberRecommandations!.toInt())}",
                                style: AppTextStyle.darkGray(14),
                              ),
                              sw(1),
                            ],
                  ...
                  sh(14),
                  _buildSignaletmentBox(context, userModel),
                  sh(14),
                  Padding(
                            ...
                            children: [
                              ...
                              Expanded(
                                child: Wrap(
                                  ...
                                  children:
                                      userModel.nationality?.map((e) {
                                        final isLast =
                                            userModel.nationality!.last == e;
                                        return Text(
                            ...
                            children: [
                              ...
                              Expanded(
                                child: Wrap(
                                  alignment: WrapAlignment.start,
                                  children:
                                      userModel.languages?.map((e) {
                                        final isLast =
                                            userModel.languages!.last == e;
                                        return Text(
                                          "${"utils.language.$e".tr()}${isLast ? "" : ", "}",
                                          style: AppTextStyle.darkGray(11),
                                        );
                  ...
                  sh(22),
                  _buildPassion(context, userModel),
                  sh(22),
                  const Divider(),
                  sh(14),
                      ...
                      if (val == 0) {
                        return FutureBuilder(
                          ...
                          builder:
                              (
                                context,
                                AsyncSnapshot<List<UserFiestaModel>?> snapshot,
                              ) {
                                if (snapshot.hasData) {
                                  if (snapshot.data == null ||
                                    ...
                                    return Center(
                                      child: Text(
                                        "app.no-fiesta-created-for-this-user"
                                            .tr(),
                                        style: AppTextStyle.darkGray(14),
                                        textAlign: TextAlign.center,
                                      ),
                                    ...
                                    return Column(
                                      children: [
                                        Text(
                                          "Soirées créées par ${userModel.firstname} (${snapshot.data!.length})",
                          ...
                          builder: (_, AsyncSnapshot<List<UserFiestaModel>?> snapshot) {
                            if (snapshot.hasData) {
                              if (snapshot.data == null ||
                                ...
                                return Center(
                                  child: Text(
                                    "app.no-fiesta-participated-for-this-user"
                                        .tr(),
                                    style: AppTextStyle.darkGray(14),
                                    textAlign: TextAlign.center,
                                  ),
  ...
  Widget _buildSignaletmentBox(BuildContext context, AppUserModel userModel) {
    if (((userModel).reportPoint ?? 0) < 50) return const SizedBox();
    final isRedFlag = ((userModel).reportPoint ?? 0) >= 100;
    return Container(
      padding: EdgeInsets.symmetric(
  ...
  Widget _buildPrincipalData(BuildContext context, AppUserModel userModel) {
    return Row(
      mainAxisSize: MainAxisSize.max,
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
                  ...
                  children: [
                    Center(
                      child: Container(
                        ...
                        child: CachedNetworkImage(
                          imageUrl: userModel.pictures!.first,
                          fit: BoxFit.cover,
                          width: formatWidth(103),
                          height: formatWidth(103),
                    ...
                    Positioned(
                      ...
                      right: 0,
                      bottom: -3,
                      child: _buildLevelButton(context, userModel),
                    ),
                  ],
                ),
              ),
        ...
        Column(
          children: [
            ...
            Button(
              ...
              // Navigation vers le chat avec l'utilisateur
              onTap: () => AutoRouter.of(
                context,
              ).navigateNamed("/dashboard/chat/${widget.id}"),
            ),
            sh(8),
            Button(
              width: formatWidth(53),
  ...
  Widget _buildLevelButton(BuildContext context, AppUserModel userModel) {
    return Row(
  ...
  Widget _buildPassion(BuildContext context, AppUserModel userModel) {
    return Wrap(
      alignment: WrapAlignment.center,
      spacing: formatWidth(9),
      runSpacing: formatWidth(9),
      children:
          (transformPassionInTags(userModel.tags ?? const PassionListing()))
              .map(
                (e) => Container(
                  padding: EdgeInsets.symmetric(
